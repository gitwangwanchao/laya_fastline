...
import QEventMgr from "./QEventMgr";
...
namespace fastline.framework
{
	export class QUtil
	{
		...
		{
			if(type==QGameConst.NumRollType.AddType)
			{
				tempData.rollValue += numPerFrame;
			}
			else if(type == QGameConst.NumRollType.ReduceType)
			{
				tempData.rollValue -= numPerFrame;
			}
			
			if ((tempData.rollValue < finalValue && type == QGameConst.NumRollType.AddType) || (tempData.rollValue >= finalValue && type == QGameConst.NumRollType.ReduceType))
			{
		...
		public static bigDivide = (str1, str2) => {
			...
			var str = QUtil.shanchuling(resultArr.join(''));
			return str;
		}
		/**
		 * 显示diaog
			...
			if(this.isStartPlay)
			{
				...
				Laya.Tween.to(btn,{scaleX:oldSX*factor,scaleY:oldSY*factor},100,Laya.Ease.linearIn,Laya.Handler.create(this,function(){
					Laya.Tween.to(btn,{scaleX:oldSX,scaleY:oldSY},100,Laya.Ease.linearIn,Laya.Handler.create(this,()=>{
						if(!QUtil.isNullOrEmpty(callback)) callback.run();
		...
		public static readFile(listRes,index ,success:any){
			tywx.wxFileUtil.readFile(listRes[index],function(){
				if (index == listRes.length-1) {
					...
					QUtil.readFile(listRes,index+1,success);
				}
			}, function(){
			});
		}
		...
		public static downloadFile(path, successFunc, failFunc){
			QUtil.removeSavedFile();
			var fileManager = Laya.Browser.window.wx.getFileSystemManager();
			var downloadTask =  Laya.Browser.window.wx.downloadFile({
				url:path,
				success:function(data){
					QDebug.D("download success!!"+JSON.stringify(data));
					tywx.BiLog.clickStat(tywx.clickStatEventType.downZipSuccess,[]);
					QDebug.D(data.tempFilePath);
					QDebug.D("--"+Laya.Browser.window.wx.env.USER_DATA_PATH);
					QUtil.unzipFile(data.tempFilePath, Laya.Browser.window.wx.env.USER_DATA_PATH+"/v" + tywx.SystemInfo.version ,successFunc);
				},fail:function(e){
					QDebug.D("download fail: "+e.errMsg);
		...
		public static unzipFile(path, targetPath,successFunc){
			...
			fileManager.unzip({
				...
				success:function(){
					...
					QDebug.D("unzip fail: "+e.errMsg);
					tywx.BiLog.clickStat(tywx.clickStatEventType.upZipFailed,[]);
					// QUtil.removeSavedFile();
					Laya.timer.once(500, this, function(){
						QUtil.unzipFile(path, targetPath,successFunc);
					});
				}
			});
		}
		...
		public static getHeightWidthRatio(): number
		{
			if (QUtil.readHeightWidthRatio == -1)
			{
				if (Laya.stage.scaleMode == Laya.Stage.SCALE_FIXED_WIDTH)
				{
					...
					QUtil.readHeightWidthRatio = realRadio / stardardRadio;
				} else
				{
					QUtil.readHeightWidthRatio = 1;
				}
			}
			return QUtil.readHeightWidthRatio;
		}
...
export default fastline.framework.QUtil;