    ...
    export class FZFlyResConfig {
        ...
        public playFlyResAni(param){
            ...
            for(let i = 0; i < countArr[this.param.countType]; i ++){
                let cNode = FZGameData.instance.getResourcesNode(this.param.type);
        ...
        public removeSelf(){
            for(let i = this.cNodeArr.length - 1; i >= 0; i--){
                FZGameData.instance.returnResourcesNode(this.cNodeArr[i],this.param.type);
            }
...
export default game.view.FZFlyResConfig;