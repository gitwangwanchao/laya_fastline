...
namespace game.view
{
    export class QShopDialog extends QBaseUI
    {
        ...
        public registerEvent() : void
        {
            ...
            // 刷新价格
            QEventMgr.instance.register(QEventType.FREE_GET_GOLD,this.onChangePrice, this);
            // 监听刷新金币
        ...
        public unregisterEvent() : void   
        {
            ...
            QEventMgr.instance.unregister(QEventType.FREE_GET_GOLD,this.onChangePrice, this);
        ...
        public init():void
        {
            ...
            Laya.timer.once(100, this, function(){  //错开动画和数据加载，避免在播放弹窗动画的时候出现卡顿
                ...
                QUtil.doUIPopAnim(this.scene.AnchorCenter);  //去掉弹窗出现的放大动画
                
                this.curMaxLevel = QMergeData.instance.getCarMaxLevel();
        ...
        public onSelect(index,f_init):void
        {
            ...
            if(!f_init){
                QSoundMgr.instance.playSfx(QSoundMgr.instance.soundInfo_wav.switch);
            }
        }
        /**
         * 道具 排序
        ...
        private ItemSort(){
            ...
            this.listItemData = [];
            var item_list = [];
            var shopItem = QCfgMgr.instance.getShopItem();
            for(let key in shopItem)
            {
                var state = 2;
                if (QMergeData.instance.getItemBuyRecord(shopItem[key].id) == true){
                    state = 3;
                } else {
                    if (shopItem[key].unlock == 0) {
                        state = 0;
                    } else if(QMergeData.instance.getItemBuyRecord(shopItem[key].unlock) == true){
                        state = 0;
                    }

                    if (state == 0) {
                        if (QUtil.StrJudge(QMergeData.instance.getGameGold()+"", shopItem[key].price+"") == 0) {
                            state = 1;
                        }
                    }
                }
            ...
            QDebug.D("排序后 --------" + JSON.stringify(this.listItemData));
        ...
        private listShopCarHandle(item,index):void {
            ...
            carPaylValue.text = QUtil.formatNumberStr(QMergeData.instance.getCarBuyPrice(this.listCarData[index].pos));
            ...
            if (itemCarLevel > this.curMaxLevel) {
                ...
                if (itemCarLevel <= this.unLockCoinLevel) {
                    ...
                    let lastVideoTime = QSavedDateItem.instance.getItemFromLocalStorage("GAME_LAST_VIDEO_BUY_CAR", "0");
                    if (QMergeData.instance.hasShopFreeCar && itemCarLevel == this.bonusCar_level && (time - parseInt(lastVideoTime)) > QMergeData.instance.SHOP_VIDEO_BUY_CAR) {
                        //视频购买
                        imgCost.skin = this.videoIconStr;
                        imgCost.scaleX = 0.6;
                        imgCost.scaleY = 0.6;
                        mtype = QGameConst.QMoneyType.Video;
                        mcost = "";
                        ...
                        mtype = QGameConst.QMoneyType.Coin;
                        mcost = QMergeData.instance.getCarBuyPrice(this.listCarData[index].pos);
                        hasCion = QUtil.StrJudge(QMergeData.instance.getGameGold()+"", mcost);
                        //btnBuy.disabled = !hasCion;
                        btnBuy.gray = !hasCion;
                        //判断打折
                        let priceProport = QMergeData.instance.getGamePriceProport();
                        if (priceProport != 1) {
                    ...
                    mtype = QGameConst.QMoneyType.Diamond;
                    mcost = QMergeData.instance.getCarBuyPriceDiamond(this.listCarData[index].pos);
                    //btnBuy.disabled = false;
                    btnBuy.gray = false;
                ...
                if (mcost != null) {
                    ...
                    lblCost.text = QUtil.formatNumberStr(mcost_str);
        ...
        onClickFreeBtnBuy(index:number,btnBuy:any){
            ...
            else
            {
                ...
                var data :any = {}; 
                    ...
                    data.state = 0; // 显示宝箱
                    // 增加车辆的购买次数
                    QMergeData.instance.addBuyCarCount(this.listCarData[index].level, QGameConst.QMoneyType.Coin);
                    // 更改车位数据
                    ...
                    this.scene.listCar.refresh();
                    QUIMgr.instance.createUI(QUIMgr.UI_Tip,{text : "购买成功"})
            }
        }
        ...
        {   
            QUtil.playBtnScaleAni(btnBuy);
            if (!btnState) {
                //QUIMgr.instance.createUI(QUIMgr.UI_Tip,{text : "车辆未解锁"})
                return ;
            }
            //车位满
            var _index = QMergeData.instance.JudgeSolt();
            if(_index == -1){
                ...
                this.isBuyCar = true;  //控制动画
                if(QMergeData.instance.coseMoney(mtype,mcost)){
                    if (mtype == QGameConst.QMoneyType.Video) {
                        let param = QShareParam.create();
                        param.shareType = QGameConst.QShareType.FreeCar;
                         // 视频
                    ...
                    data.state = 0; // 显示宝箱
                    // 增加车辆的购买次数
                    QMergeData.instance.addBuyCarCount(this.listCarData[index].level, mtype);
                    // 更改车位数据
                    QMergeData.instance.changeCarSlotData(_index, data);
                    //更新车的位置
                    QEventMgr.instance.sendEvent(QEventType.MAIN_VIEW_TOUCH_UP_CAR_INDEX, {"index": _index});
        ...
        shareCallBack(item_index: number, solt_index: number) {
            ...
            solt_index = QMergeData.instance.JudgeSolt();
            if(solt_index == -1){
                QUIMgr.instance.createUI(QUIMgr.UI_Tip,{text : "车位已满，请先合成或回收车辆"})
                return;
            }
            let time = new Date().getTime();
            QSavedDateItem.instance.setItemToLocalStorage("GAME_LAST_VIDEO_BUY_CAR", time+"");

            var data :any = {}; 
            data.level = this.listCarData[item_index].level;
            data.state = 0; // 显示宝箱
            // 增加车辆的购买次数
            QMergeData.instance.addBuyCarCount(this.listCarData[item_index].level, QGameConst.QMoneyType.Video);
            // 更改车位数据
            QMergeData.instance.changeCarSlotData(solt_index, data);
            //更新车的位置
            QEventMgr.instance.sendEvent(QEventType.MAIN_VIEW_TOUCH_UP_CAR_INDEX, {"index": solt_index});
            this.scene.listCar.refresh();
        ...
        private listShopPropHandle(item,index):void
        {
            ...
            lblCost.text = QUtil.formatNumberStr(itemdata.price + "");
            ...
            btnBuy.on(Laya.Event.CLICK,this,this.onClickBtnBuyItem,[index,QGameConst.QMoneyType.Coin,itemdata.price,btnBuy, itemdata.state]);
        }
        ...
        private onClickBtnBuyItem(index:number,mtype:number,mcost:number,btnBuy:any, btnState:number):void
        {
            ...
            if (btnState == 1) {
                ...
                QUIMgr.instance.createUI(QUIMgr.UI_Tip,{text : "道具未解锁"});
            } 
            else{
                if(QMergeData.instance.coseMoney(mtype,mcost)) {
                    if(this.listItemData[index].type == 1){
                        ...
                        QMergeData.instance.setGameEarnings(count);
                    }
                    else if (this.listItemData[index].type == 2){
                        // 价格
                        var count = 1 - this.listItemData[index].value/100;
                        QMergeData.instance.setGamePriceProport(count);
                    }
                    QMergeData.instance.addItemBuyRecord(this.listItemData[index].id);
                    this.ItemSort();
        ...
        private onClickBtnClose():void
        {
            ...
            Laya.timer.once(310, this, function(){
                QUIMgr.instance.removeUI(QUIMgr.UI_ShopDialog);
            });
        }
    }
}
export default game.view.QShopDialog;