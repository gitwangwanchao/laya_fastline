...
namespace game.view
{
    export class QBullet extends QBaseNode
    {
        ...
        public startFire(params:any):void
        {
            ...
            if(params.type == "QEnemyBossBullet") {
                ...
                if (!QUtil.isNullOrEmpty(params.bullet_data)) {
            ...
            this.speed_x = Math.sin(this.scene.rotation * (Math.PI/180))*(QGameData.instance.getMapSpeed() - this.speed_running);
            this.speed_y = Math.cos(this.scene.rotation * (Math.PI/180))*(QGameData.instance.getMapSpeed() - this.speed_running);
            ...
            this.TIME = QGameData.instance.game_running_time;
            if (QGameData.instance.game_stop == true) {
                this.TIME = QGameData.instance.game_stop_time;
            }
        ...
        public onGameOver() {
            if (this.scene.visible == true) {
                this.bullet_state = 0;
                this.onUpdateBoxPosit();
                QGameData.instance.killPoolNodeList(this);
            }
        }
        /**
         *  结束子弹
        ...
        public playDeathAni():void
        {
            ...
            if(this.bz_ani && (this.bulletType == "QAirCraftBullet_left" || this.bulletType == "QAirCraftBullet_right"))
            {
                QGameData.instance.playExplosion({type:2,x:this.scene.x + this.scene.img_bullet.x, y:this.scene.y + this.scene.img_bullet.y});
            ...
            {
                QGameData.instance.playExplosion({type:1,x:this.scene.x + this.scene.img_bullet.x, y:this.scene.y + this.scene.img_bullet.y});
        ...
        public UpdatePos():void {
            ...
            if (this.bullet_state == 1) {
                this.scene.y  -= (this.speed_y - this.change_data.pos_y)* QGameData.instance.getGameTime();
                this.scene.x  += (this.speed_x + this.change_data.pos_x)* QGameData.instance.getGameTime();
                this.onUpdateBoxPosit();
            }
            if(this.bullet_state == 2) {
                // this.scene.y  += this.speed_y * QGameData.instance.getGameTime();
                // this.scene.x  += this.speed_x * QGameData.instance.getGameTime();
                this.onUpdateBoxPosit();
            }
        }
         /**
        ...
        {
            // 获取目标 
            if (this.enemy_js != null && QGameData.instance.judgeEnemyDeath(this.enemy_js) == 1) {
                // 存在目标点
                var rota = QUtil.getAngle(this.scene.x , this.scene.y , this.enemy_js.scene.x, this.enemy_js.scene.y);
                return rota;
            } else {
                // 重新选定目标
                var data :any = QGameData.instance.getEnemyNearly();
                if (data == -1){
                    this.enemy_js = null;
                    // 没有目标
                    return 0;
                    ...
                    this.enemy_js = data;
                    // 存在目标
                    var rota = QUtil.getAngle(this.scene.x , this.scene.y , data.scene.x, data.scene.y);
                    return rota;
                }
            }
        }
...
export default game.view.QBullet;