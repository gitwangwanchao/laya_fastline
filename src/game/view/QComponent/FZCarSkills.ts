...
namespace game.view
{
    export class FZCarSkills extends FZBaseNode
    {
        ...
        public playSkill(params:any):void
        {
            ...
            FZDebug.D("播放技能----------------------" + JSON.stringify(params));
            if (this.skilltype == 1003) {
                this.pos_end =  (this.getPosX(FZGameData.instance.getFZPlayerUIPos().x) - this.getPosX(params.x))/100;
                ...
                this.skillstate = 3;
                this.scene.y = 0;
                this.scene.x = this.getPosX(FZGameData.instance.getFZPlayerUIPos().x);
        ...
        public getPosX(posx){
            var car_pos = FZGameData.instance.car_pos;
        ...
        public UpdatePos():void {
            ...
            FZDebug.D("UpdatePos刷新技能 位置-------Y   = " + this.scene.y);
            if (this.skilltype == 1003) {
        ...
        public skill1UpdatePos(){
            if (this.updatePosState == 1) {
                // 爆炸
                this.scene.y += this.skill1speed * FZGameData.instance.getGameTime();
                // this.pos_end 
            }else if (this.updatePosState == 2) {
                this.scene.y += this.skillspeed["1"] * FZGameData.instance.getGameTime();
            }
        }
        public skill3UpdatePos() {
            if (this.skillstate == 1){
                this.scene.y += this.skillspeed["3"] * FZGameData.instance.getGameTime();
            }
        }
         /**
         * 判断是否移出屏幕 
        ...
        public onJudgeRemove(){    
            if (this.scene.y > 2000) {
                this.skillstate = 0;
                FZGameData.instance.killSkillPoolList(this);
            }
        }
        ...
        public skill2UpdatePos(){
            // 爆炸
            this.scene.y += this.skillspeed["2"] * FZGameData.instance.getGameTime();
        }
...
export default game.view.FZCarSkills;