...
module game.core
{
	export class FZSceneManager
	{
        private constructor() { }

		private static _instance: FZSceneManager;
		public static get instance(): FZSceneManager
		{
			if (FZSceneManager._instance == null)
			{
				FZSceneManager._instance = new FZSceneManager();
        ...
        {
			FZEventManager.instance.sendEvent(FZEvent.RES_LOAD_FINISHED, FZGameStatus.QResType.GarageScene);
			...
			FZUtils.setCameraFov(garageCarCamera);
		...
		public changeCurUseCar(){
			let curUseCarLevel = FZMergeDateManager.instance.getCarUsedLevel();
			curUseCarLevel = Math.max(curUseCarLevel, 1);
			curUseCarLevel = Math.min(curUseCarLevel, 50);
			var car_data = FZCfgManager.instance.getCarInfoById(curUseCarLevel);
			var weapon_data = FZCfgManager.instance.getMainWeapons(car_data.mainWeaponId);				
			for (var i=1;i <=50;i++) {
				if (curUseCarLevel == i) {
		...
		public loadUISceneFinish():void {
			...
			camera.orthographicVerticalSize = 12 * FZUtils.getHeightWidthRatio();
			this.uiScene.active = false;
			Laya.stage.addChild(this.uiScene);
		}
	}
}
export default game.core.FZSceneManager;