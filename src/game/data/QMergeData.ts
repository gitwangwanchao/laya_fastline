import QSavedDateItem from "./QSavedDateItem";
...
import QEventType from "./QEventType";
import QGameData from "./QGameData";
import QUIMgr from "../core/QUIMgr";
import QGameConst from "./QGameConst";
import QHttp from "../../framework/QHttp";
import QDBullet from "../view/QDBullet";
...
namespace game.data
{
    export class QMergeData
    {
        ...
        private static _instance: QMergeData;
        public static get instance(): QMergeData
        {
            if (this._instance == null)
            {
                this._instance = new QMergeData();
        ...
        public setLvUpGuideTouch(){
            let dt = QSavedDateItem.instance.setItemToLocalStorage("lvUpGuideTouch","yes");
        }
        ...
        addUserExperience(exp: number):void{
            ...
            var info = QCfgMgr.instance.getLevelInfo(this.user_level);
            if (info.level_up_exp == -1) {
                return;
            }
            if (this.user_experience >= info.level_up_exp) {
                this.user_level += 1;
                QSavedDateItem.instance.setItemToLocalStorage("USER_LEVEL", this.user_level.toString());
                 // 升级
                QEventMgr.instance.sendEvent(QEventType.MAIN_VIEW_UPDATE_LEVEL);
                var newinfo = QCfgMgr.instance.getLevelInfo(this.user_level)
            ...
            // 更新 经验
            QEventMgr.instance.sendEvent(QEventType.MAIN_VIEW_UPDATE_EXPERIENCE);
        ...
        public setCarUsedLevel(level: number){
            if (this.CarUsedLevel != level) {
                ...
                Laya.timer.frameOnce(2, this, function(){
                    QSavedDateItem.instance.setItemToLocalStorage("CARUSEDLEVEL", this.CarUsedLevel.toString());
                });
                QEventMgr.instance.sendEvent(QEventType.GAME_CHANGE_CUR_USE_CAR);
            }
        ...
        {
            QDebug.D("合成设置等级  ------  " + level );
            QDebug.D("合成设置等级  ------  " + this.CarMaxLevel );
            if (level > this.CarMaxLevel) {
                this.CarMaxLevel = level;
                Laya.timer.frameOnce(2, this, function(){
                    QSavedDateItem.instance.setItemToLocalStorage("CARMAXLEVEL", this.CarMaxLevel.toString());
                })
                this.setCarUsedLevel(level);//升级后将当前选中车改成新车
                // 更改选择
                QEventMgr.instance.sendEvent(QEventType.MAIN_VIEW_MERGE_NEW_CAR);
                if (level == QGameData.instance.deputyWeapon_open_point) {
                    QGameData.instance.setDeputyWeaponLocalLevel(1);
                }
                 // 上传网络数据
                QMergeData.instance.sendUpdataData();
                if(this.CarMaxLevel  == QMergeData.instance.curTableCarMaxLevel) {
        ...
        initData(): void 
        {
            ...
            this.haveInfo = QSavedDateItem.instance.getItemFromLocalStorage("GAME_DATA","");
            if(!this.haveInfo){
                //覆盖本地数据
        ...
        public initPopJcdlCfg(){
            ...
            this.appIDClicked = JSON.parse(QSavedDateItem.instance.getItemFromLocalStorage("APPID_CILCK_ARR", JSON.stringify([])));
            var sameDay = this.isSamedayJcdl();
            if(sameDay){
                this.jcdlCfgArr = this.getPopJcdlCfgStorage();  //获取弹窗交叉导流的本地配置信息
                this.updatePopJcdlWhenVersionUp();
            }else{
                QSavedDateItem.instance.setItemToLocalStorage("MORE_GAME_DAY_FRESH", "1");
                this.fleshPopJcdl();
            }
        }
        ...
        public changePopJcdlCfg(param){
            if(this.jcdlCfgArr.length > 0){
                ...
                QSavedDateItem.instance.setItemToLocalStorage("APPID_CILCK_ARR", JSON.stringify(this.appIDClicked));
                this.setPopJcdlCfgStorage(this.jcdlCfgArr);
                QEventMgr.instance.sendEvent(QEventType.MORE_GAME_NUM);
            }
        }
        ...
        public isSamedayJcdl(){
            ...
            if(day != QSavedDateItem.instance.getItemFromLocalStorage("MORE_GAME_DAY", "0")){
                //不是同一天
                // this.fleshPopJcdl();
                // QSavedDateItem.instance.setItemToLocalStorage("MORE_GAME_DAY_FRESH", "1");  //刷新可试玩次数
        ...
        public getPopJcdlCfgStorage(){
            let cfg = QSavedDateItem.instance.getItemFromLocalStorage("POP_JCDL_CFG", "");
            if(cfg){
                return JSON.parse(QSavedDateItem.instance.getItemFromLocalStorage("POP_JCDL_CFG", ""));
            }else{
                return [];
            }
        }
        ...
        public setPopJcdlCfgStorage(param){
            QSavedDateItem.instance.setItemToLocalStorage("POP_JCDL_CFG", JSON.stringify(param));
        }
         ...
         */
        dateStorage(day){
            QSavedDateItem.instance.setItemToLocalStorage("MORE_GAME_DAY", day);
        }
        ...
        public setLaunchOptionsSync():any
		{
			if(Laya.Browser.onMiniGame)
			{
                ...
                QDebug.log("info : " + tywx.UserInfo.scene_id);
				if(!QUtil.isNullOrEmpty(tywx.UserInfo.scene_id)){
					//视频本地策略
					...
					if(spArr.indexOf(parseInt(tywx.UserInfo.scene_id)) >= 0/* || (info.query.fun_type && info.query.fun_type == "onShareAppMessage")*/){
						QGameData.instance.setOnlyCanVideo(true);
					}else{
						QGameData.instance.setOnlyCanVideo(false);
					}
				}
			}
		}
        ...
        setLocalGameData(): void{
            // 钻石
            this.game_diamond = parseInt(QSavedDateItem.instance.getItemFromLocalStorage("GAME_DIANOND", "0")); 
            if (this.game_diamond < 0) {
                this.game_diamond = 0;
            }
            // 车辆最高等级
            this.CarMaxLevel = parseInt(QSavedDateItem.instance.getItemFromLocalStorage("CARMAXLEVEL","1"));
            // todo
            this.CarMaxLevel = Math.min(50,this.CarMaxLevel);
            // 当前使用赛车等级
            this.CarUsedLevel = parseInt(QSavedDateItem.instance.getItemFromLocalStorage("CARUSEDLEVEL","1"));
            this.CarUsedLevel = Math.min(50, this.CarUsedLevel);
            // 1 用户拥有的金币
            this.game_gold = QSavedDateItem.instance.getItemFromLocalStorage("GAME_GOLD","4000");
             // var n = this.game_gold.split('NaN').length-1;
            //  if (this.game_gold.indexOf("NaN") != -1) {
            //     this.game_gold = "10000000000000000000000";
            // }
            // 开启的车位
            this.game_card_slot = parseInt(QSavedDateItem.instance.getItemFromLocalStorage("GAME_CARD_SLOT","12"));
            // 车位数据
            this.game_card_slot_data = JSON.parse(QSavedDateItem.instance.getItemFromLocalStorage("GAME_CARD_SLOT_DATA", JSON.stringify({"0":{"level": 1, "state":1}})));
            ...
            QSavedDateItem.instance.setItemToLocalStorage("CARMAXLEVEL",this.CarMaxLevel.toString());
            // 购买记录 金币
            this.game_buy_record = JSON.parse(QSavedDateItem.instance.getItemFromLocalStorage("GAME_BUG_RECORD", "{}"));
            // 购买记录 钻石
            this.game_buy_diamond_record = JSON.parse(QSavedDateItem.instance.getItemFromLocalStorage("GAME_BUG_DIAMOND_RECORD", "{}"));
            // 当前经验
            this.user_experience = parseInt(QSavedDateItem.instance.getItemFromLocalStorage("USER_EXPERIENCE","0"));
            // 用户等级
            this.user_level = parseInt(QSavedDateItem.instance.getItemFromLocalStorage("USER_LEVEL","1"));
            // 游戏金币速率
            this.game_gold_speed = this.onUpdateGoldSpeed();
            // 永久增益
            this.game_earnings = parseInt(QSavedDateItem.instance.getItemFromLocalStorage("GAME_EARINGS","1"));
            // 加速时间
            var last_time = JSON.parse(QSavedDateItem.instance.getItemFromLocalStorage("GAME_SPEED_LAST_LIST", JSON.stringify({"2": -1, "5": -1})));
            // 合成软引导
            this.syntheticSoft = parseInt(QSavedDateItem.instance.getItemFromLocalStorage("syntheticSuccess","0"));
            // 商店软引导
            this.shopSoft = parseInt(QSavedDateItem.instance.getItemFromLocalStorage("shopTouch","0"));
            // 未操作时长
            this.noTouchTime = parseInt(QSavedDateItem.instance.getItemFromLocalStorage("noTouchTime","0"));
            // 今日分享次数
            this.todayShareCount = parseInt(QSavedDateItem.instance.getItemFromLocalStorage("todayShareCount", "1"));
            // 上次分享的日期
            this.shareCountDate = QSavedDateItem.instance.getItemFromLocalStorage('shareCountDate',"");
            ...
            // 道具购买记录
            this.buy_item_record = JSON.parse(QSavedDateItem.instance.getItemFromLocalStorage("BUY_ITEM_RECORD",JSON.stringify([])));
            // 价格比例
            this.game_price_proport = parseFloat(QSavedDateItem.instance.getItemFromLocalStorage("GAME_PRICE_PROPORT","1"));
            //是否第一次回收车辆
            this.is_First_Recycle_Car = parseFloat(QSavedDateItem.instance.getItemFromLocalStorage("IS_FIRST_RECYCLE_CAR","0"));
            //是否第一次点击转盘抽奖
            this.is_First_Click_Lucky = parseFloat(QSavedDateItem.instance.getItemFromLocalStorage("IS_FIRST_CLICK_LUCKY","0"));
            //游戏胜利次数
            this.play_game_win_times = parseFloat(QSavedDateItem.instance.getItemFromLocalStorage("PLAY_GAME_WIN_TIMES","0"));
            // 初始化 空投包数据
        ...
        {
            new QHttp().get("game/getUserAccScore",this,(e)=>{
                let dt = e.data;
                if(e.state == 200 && dt)
                {
                    this.userIntegral = dt.accScore;
                    QDebug.D("用户分数------------------------ = " + this.userIntegral);
                }
            },null);
        }
        ...
        public setUseriInviteCode()
        {
            ...
            {
                new QHttp().get("game/setInviteInfo",this,(e)=>{
        ...
        public getUserInfo(): any{
            if((Laya.Browser.onMiniGame || Laya.Browser.onAndroid || Laya.Browser.onIOS)) {
                if (!this.clearAccount) {
                    QDebug.D("请求用户数据------------------------");
                    new QHttp().get("game/getGameInfo",this,this.getAIWxUserDataSuccess,null);
                } 
            }
        }
        /**
        ...
        public judgeData(value):any{
            if (this.CarMaxLevel < parseInt(value.carMaxLevel) || QGameData.instance.getMaxCheckPoint() < parseInt(value.maxCheckPoint)
            || QGameData.instance.getMainWeaponLevel() < parseInt(value.mainWeaponLevel) || QGameData.instance.getDeputyWeaponLocalLevel() < parseInt(value.deputyWeaponLevel)) {
                return true;
            }else {
                if (this.CarMaxLevel == parseInt(value.carMaxLevel) || QGameData.instance.getMaxCheckPoint() == parseInt(value.maxCheckPoint)
                || QGameData.instance.getMainWeaponLevel() == parseInt(value.mainWeaponLevel) || QGameData.instance.getDeputyWeaponLocalLevel() == parseInt(value.deputyWeaponLevel)) {
        ...
        public getAIWxUserDataSuccess(e): void{
            ...
            if(e.state == 200 && dt && dt.gamedata){
                ...
                QSavedDateItem.instance.setItemToLocalStorage("GAME_DIANOND",this.game_diamond.toString()); 
                ...
                QSavedDateItem.instance.setItemToLocalStorage("USER_LEVEL", this.user_level.toString());
                QSavedDateItem.instance.setItemToLocalStorage("GAME_CARD_SLOT","12");
                // 车位数据
                QSavedDateItem.instance.setItemToLocalStorage("GAME_CARD_SLOT_DATA", JSON.stringify(this.game_card_slot_data));
                // 购买记录 金币
                QSavedDateItem.instance.setItemToLocalStorage("GAME_BUG_RECORD", JSON.stringify(this.game_buy_record));
                // 购买记录 钻石
                QSavedDateItem.instance.setItemToLocalStorage("GAME_BUG_DIAMOND_RECORD", JSON.stringify(this.game_buy_diamond_record));
                // 当前经验
                QSavedDateItem.instance.setItemToLocalStorage("USER_EXPERIENCE",this.user_experience.toString());
                QSavedDateItem.instance.setItemToLocalStorage("GAME_EARINGS",this.game_earnings.toString());
                QSavedDateItem.instance.setItemToLocalStorage("BUY_ITEM_RECORD",JSON.stringify(this.buy_item_record));
                QSavedDateItem.instance.setItemToLocalStorage("GAME_PRICE_PROPORT",this.game_price_proport.toString());
                QSavedDateItem.instance.setItemToLocalStorage("IS_FIRST_RECYCLE_CAR",this.is_First_Recycle_Car.toString());
                QSavedDateItem.instance.setItemToLocalStorage("IS_FIRST_CLICK_LUCKY",this.is_First_Click_Lucky.toString());
                QSavedDateItem.instance.setItemToLocalStorage("PLAY_GAME_WIN_TIMES",this.play_game_win_times.toString());
                // 用户等级
                // gameData
                QGameData.instance.setWeaponsCoin(gamedata.weaponCoin || 0);
                QGameData.instance.setMainWeaponLevel(gamedata.mainWeaponLevel || 1);
                QGameData.instance.setCheckPoint(gamedata.checkPoint || 1),
                QGameData.instance.setMaxCheckPoint(gamedata.maxCheckPoint || 1),
                QGameData.instance.setDeputyWeaponLocalLevel(gamedata.deputyWeaponLevel || 0);
                QGameData.instance.setCurUseUAV(gamedata.curSelectUav || -1);
                QGameData.instance.setUAVData(gamedata.uavData);
                QGameData.instance.newPlayerGudieStep(gamedata.guideStep);
                if(!QUtil.isNullOrEmpty(gamedata.AirDropList)) {
                    this.AirDropList = gamedata.AirDropList;
                    QSavedDateItem.instance.setItemToLocalStorage("QMergeDateAirDropList",JSON.stringify(this.AirDropList)); 
                }
                if(!QUtil.isNullOrEmpty(gamedata.AirDropIsOpen)) {
                    ...
                    QSavedDateItem.instance.setItemToLocalStorage("QMergeDateAirDropIsOpen", this.AirDropIsOpen + ""); 
                }
                if(!QUtil.isNullOrEmpty(gamedata.FreeAirDropOpen)) {
                    this.FreeAirDropOpen = gamedata.FreeAirDropOpen;
                    QSavedDateItem.instance.setItemToLocalStorage("QMergeDateFreeAirDropOpen", this.FreeAirDropOpen + ""); 
                }
                if(!QUtil.isNullOrEmpty(gamedata.AirDropCount)) {
                    this.AirDropCount = gamedata.AirDropCount;
                    if(this.AirDropIsOpen == 2){
                        this.AirDropCount = 0;
                        QSavedDateItem.instance.setItemToLocalStorage("QMergeDateAirDropCount", this.AirDropCount + ""); 
                        // todo 
                        this.AirDropTime = 120;
                        QSavedDateItem.instance.setItemToLocalStorage("QMergeDateAirDropTime", this.AirDropTime + "");
                    ...
                    QEventMgr.instance.sendEvent(QEventType.UPDATE_AIRDROP_RED_NUM);
                }
                // 处理异常
                ...
                if(!QUtil.isNullOrEmpty(gamedata.AirdropPos)) {
                    this.AirdropPos = gamedata.AirdropPos;
                    QSavedDateItem.instance.setItemToLocalStorage("QMergeDateAirDropPos", JSON.stringify(this.AirdropPos)); 
                }
                if(!QUtil.isNullOrEmpty(gamedata.airDrop_promptly_show)){
                    QGameData.instance.airDrop_promptly_show = gamedata.airDrop_promptly_show;
                    QGameData.instance.setAirDropInstance(gamedata.airDrop_promptly_show)
                }


                if (QUtil.isNullOrEmpty(this.haveInfo)){
                    QSavedDateItem.instance.setItemToLocalStorage("GAME_DATA",'haveInfo');  
                    this.haveInfo = "haveInfo";      
                }

                
                this.NetWorkOK = 1;
                QEventMgr.instance.sendEvent(QEventType.MAIN_VIEW_UPDATE_UI);
        ...
        public sendUpdataData() {
            ...
            let data = {
                ...
                // gameData
                weaponCoin: QGameData.instance.getWeaponsCoin(),
                mainWeaponLevel: QGameData.instance.getMainWeaponLevel(),
                checkPoint: QGameData.instance.getCheckPoint(),
                maxCheckPoint: QGameData.instance.getMaxCheckPoint(),
                deputyWeaponLevel: QGameData.instance.getDeputyWeaponLocalLevel(),
                curSelectUav: QGameData.instance.getCurUseUAV(),
                uavData: QGameData.instance.getUAVData(),
                guideStep: QGameData.instance.newPlayerGudieStep(null),
                ...
                airDrop_promptly_show: QGameData.instance.airDrop_promptly_show
            }
        ...
        public setGameData(data): any{
            new QHttp().get("game/setGameInfo",this,(e)=>{
                QDebug.D("数据同步成功---------------------------");
            },data);
        }
        ...
        {
            QDebug.D("设置永久收益1  " + this.game_earnings);
            this.game_earnings = value;
            QDebug.D("设置永久收益2  " + this.game_earnings);
        ...
        public setGamePriceProport(value:number):void
        {
            ...
            QEventMgr.instance.sendEvent(QEventType.MAIN_VIEW_CHANGE_PRICE);
        }
        ...
        addGameDiamond(value:number):any 
        {
            ...
            QSavedDateItem.instance.setItemToLocalStorage("GAME_DIANOND",this.game_diamond.toString()); 
            QEventMgr.instance.sendEvent(QEventType.MAIN_VIEW_UPDATE_DIAMOND);// 刷新钻石
            // 上传网络数据
            QMergeData.instance.sendUpdataData();
        }
        /**
        ...
        private onUpdateGoldSpeed():any {
            var speed = 0;
            // QDebug.D("更新金币速率---------------------" + JSON.stringify(this.game_card_slot_data));
            for (var k in this.game_card_slot_data) {
                if (!QUtil.isNullOrEmpty(this.game_card_slot_data[k])) {
                    var level = this.game_card_slot_data[k].level;
                    var info = QCfgMgr.instance.getCarInfoById(level);
            ...
            QEventMgr.instance.sendEvent(QEventType.MAIN_VIEW_UPDATE_GOLD_SPEED);
            return this.game_gold_speed;
        }
        ...
        public getCarSlotOutputByTime(time: number){
            ...
            for (var k in this.game_card_slot_data) {
                if (!QUtil.isNullOrEmpty(this.game_card_slot_data[k])) {
                    ...
                    //gold += Math.ceil(times * info.output_gold * QMergeData.instance.getGameEarnings());
                }
            }
            return gold;
        }
        ...
        initTest()
        {
            QSavedDateItem.instance.setItemToLocalStorage("USER_LEVEL","1");
            QSavedDateItem.instance.setItemToLocalStorage("CARMAXLEVEL", "1");
            QSavedDateItem.instance.setItemToLocalStorage("CARUSEDLEVEL", "1");
            // 保存金币
            QSavedDateItem.instance.setItemToLocalStorage("GAME_GOLD","1000000");
            // 开启的车位
            QSavedDateItem.instance.setItemToLocalStorage("GAME_CARD_SLOT","12");
            // 车位数据
            QSavedDateItem.instance.setItemToLocalStorage("GAME_CARD_SLOT_DATA", JSON.stringify({"0":{"level": 1, "state":1}}));
            // 购买记录
            QSavedDateItem.instance.setItemToLocalStorage("GAME_BUG_RECORD", "{}");
            // 保存当前经验
            QSavedDateItem.instance.setItemToLocalStorage("USER_EXPERIENCE","0");
            // 购买道具记录
            QSavedDateItem.instance.setItemToLocalStorage("BUY_ITEM_RECORD",JSON.stringify([]));
            // 打折卡 
            QSavedDateItem.instance.setItemToLocalStorage("GAME_PRICE_PROPORT","1")
        }
        ...
        public addGameGold(value: string): void {
            value =  QUtil.scienceNum(value);
            this.game_gold = QUtil.StrAdd(this.game_gold, value);
            QEventMgr.instance.sendEvent(QEventType.MAIN_VIEW_UPDATE_GAME_GOLD);
        }
        ...
        public SubtractGameGold(value: string): void {
            value =  QUtil.scienceNum(value);
            this.game_gold = QUtil.StrSubtract(this.game_gold, value);
            QDebug.D("减少游戏金币---------" + this.game_gold);
            QEventMgr.instance.sendEvent(QEventType.MAIN_VIEW_UPDATE_GAME_GOLD);
        }
        ...
        public StrMultipGameGold(value:number): void {
            this.game_gold = QUtil.StrMultip(this.game_gold, value.toString());
            QEventMgr.instance.sendEvent(QEventType.MAIN_VIEW_UPDATE_GAME_GOLD);
        }
        ...
        public getGameCarSlotPos(index: number):any 
        {
            ...
            if (QUtil.isNullOrEmpty(pos_list)) {
                QDebug.E("getGameCarSlotPos :" + "参数异常");
                return null;
            }
            return pos_list;
        }
        ...
        public getCarBuyPrice(level : number): string {
            var car_info = QCfgMgr.instance.getCarInfoById(level);
            var pow = this.getBuyCarCount(level, QGameConst.QMoneyType.Coin);
         ...
         public getCarBuyPriceDiamond(level : number): string {
            var car_info = QCfgMgr.instance.getCarInfoById(level);
            if(!car_info) {return '';}
            var pow = this.getBuyCarCount(level, QGameConst.QMoneyType.Diamond);
        ...
        public getCarSellPrice(level :number): string {
            ...
            var gold = QUtil.scienceNum(sell_gold);
            return gold;
        } 
        ...
        public  getCheapestCar(): any {
            //旧算法
            /*let maxBuyGunLevel = this.CarMaxLevel - QMergeData.instance.MaxBuyGunLevelDiv;
            maxBuyGunLevel = maxBuyGunLevel < 0 ? 0 : maxBuyGunLevel;
            var level = 1;
            if (0 != maxBuyGunLevel) {
                //区间
                let price = this.getCarBuyPrice(1);
                QDebug.D("获取最优车辆 -----------0-price=  " + price);
            ...
            if (this.CarMaxLevel > 5) {
                let minBuyLevel = (this.CarMaxLevel <= 9) ? 1 : this.CarMaxLevel - (QMergeData.instance.MaxFastBuyCarLevelDiv + 4 - 1);
                ...
                for (let i = minBuyLevel; i <= maxBuyLevel; i++) {
                    price = this.getCarBuyPrice(i);
                    let unitPrice = QUtil.bigDivide(price , Math.pow(2, i).toString()); //单价
                    if (QUtil.StrJudge(minUnitPrice , unitPrice) == 1) {
                        minUnitPrice = unitPrice;
                        level = i;
                    }
                }
        ...
        public JudgeSolt():number {
            // QDebug.D("判断哪个车位空闲 ------------------" + JSON.stringify(this.game_card_slot_data));
            for (var k in this.game_card_slot_data) {
                if (this.game_card_slot_data[k]!= null){
                    if (this.game_card_slot_data[k].level == null || this.game_card_slot_data[k].level <= 0) {
                        this.game_card_slot_data[k]= null;
            ...
            for(var i = 0; i < this.game_card_slot; i++ ) {
                if (QUtil.isNullOrEmpty(this.game_card_slot_data[i + ""])) {
                    return i;
                }
            }
            return -1;
        ...
        public isLoadLastIndex() : void {
            if ( QGameData.instance.deleteCarGuide == "Close") {
                return;
            }
            var carNum = 0;
            for(var i = 0; i < this.game_card_slot; i++ ) {
            ...
            if( carNum == 1 ){  
                QEventMgr.instance.sendEvent(QEventType.DELETE_CAR_TRASH_GUIDE); // 删除车辆引导
            }
        }
            ...
            if (this.showRandomCar) {
                ...
                for ( var i = 0; i < time; i++){
                    ...
                    if (_index == -1) {
                        ...
                        var level = QMergeData.instance.getRandomCarDate();
                        if (level != -1) {
                            var data :any = {}; 
                            data.level = level;
        ...
        public addOffLineGold(popSign: boolean = false) {
            var offlinetime = parseInt(QSavedDateItem.instance.getItemFromLocalStorage("GAME_OFFLINE_TIME", "0"));
            // console.log("addOffLineGold 计算离线收益 --- offlinetime = " + offlinetime);
            if (offlinetime == 0 ) {
                // 第一次进入游戏
                var time_1 = new Date().getTime();
                QSavedDateItem.instance.setItemToLocalStorage("GAME_OFFLINE_TIME",time_1.toString());
                return;
            }
            var maxTime = QCfgMgr.instance.getOffLineMaxTime();
            var newtime = new Date().getTime();
            ...
            var data = QCfgMgr.instance.getOffLine();
            var gold = 0;
            for (var k = 0; k <=8 ;k++) {
                var key = k + "";
            ...
            QDebug.D("离线界面 小时------------------- "  + offline_h);
            QDebug.D("离线界面 秒------------------- "  + offline_s);

            gold = Math.floor(gold);
            var time = QCfgMgr.instance.getOffLineTime();
            ...
            {
                QDebug.D("离线界面 ------------------1- "  + gold);
                // 弹窗离线界面
                if (!popSign) {
                    QUIMgr.instance.createUI(QUIMgr.UI_BeOffline, {"gold": gold});
                } else {
                    QUIMgr.instance.addWaitUI(QUIMgr.UI_BeOffline, {"gold": gold});
                }
                return true;
            }else {
                QDebug.D("离线界面 ------------------2- "  + gold);
                this.addGameGold(gold + "");
            }
        }
        /**
        ...
        public onShow() 
        {
            // var lastTime:any = JSON.parse(QSavedDateItem.instance.getItemFromLocalStorage("GAME_SPEED_LAST_LIST", JSON.stringify({"2":-1, "5":-1})));
            // var newtime = new Date().getTime();
            ...
            // Laya.timer.resume();
            // 计算离线金币
            if(QUIMgr.instance.getDialogActive(QUIMgr.UI_Main)){
                this.addOffLineGold();
            }
        }
        ...
        public onHide()
        {
            ...
            QDebug.D("切换到后台 -------------------------" + JSON.stringify(lastTime));
            QSavedDateItem.instance.setItemToLocalStorage("GAME_SPEED_LAST_LIST", JSON.stringify(lastTime));
            if (!this.clearAccount) {
                this.saveAllLocalData();
                QGameData.instance.saveAllLocalData();
            }
            ...
            // Laya.timer.pause();
            // 存储离线时间
            QSavedDateItem.instance.setItemToLocalStorage("GAME_OFFLINE_TIME", newtime.toString());
        }
         /**
         * 存储本地数据
         */
        public saveAllLocalData():void {
            ...
            // QSavedDateItem.instance.setItemToLocalStorage("CARUSEDLEVEL", this.CarUsedLevel.toString());
            // 保存金币
            QSavedDateItem.instance.setItemToLocalStorage("GAME_GOLD",this.game_gold);
            // 开启的车位
            // QSavedDateItem.instance.setItemToLocalStorage("GAME_CARD_SLOT",this.game_card_slot.toString());
            // 车位数据
            // QSavedDateItem.instance.setItemToLocalStorage("GAME_CARD_SLOT_DATA", JSON.stringify(this.game_card_slot_data));
            // 购买记录 金币
            // QSavedDateItem.instance.setItemToLocalStorage("GAME_BUG_RECORD", JSON.stringify(this.game_buy_record));
            ...
            // QSavedDateItem.instance.setItemToLocalStorage("GAME_PRICE_PROPORT",this.game_price_proport.toString());
            // 合成软引导
            QSavedDateItem.instance.setItemToLocalStorage("syntheticSuccess",this.syntheticSoft.toString());
            // 商店软引导
            QSavedDateItem.instance.setItemToLocalStorage("shopTouch",this.shopSoft.toString());
            // 时间
            QSavedDateItem.instance.setItemToLocalStorage("noTouchTime",this.noTouchTime.toString());
            // 今日分享的次数(不传服务器)
            QSavedDateItem.instance.setItemToLocalStorage("todayShareCount",this.todayShareCount.toString());
            // 上次分享的日期
            QSavedDateItem.instance.setItemToLocalStorage('shareCountDate',this.shareCountDate.toString());
        }
        ...
        public initShareCount(){
            ...
            if(this.shareCountDate != date_str)
            {
                ...
                QSavedDateItem.instance.setItemToLocalStorage("shareCountDate", date_str);
            }
        }
        ...
        addGameSpeedTime(key:number, value:number):void
        {
            ...
            QDebug.D("增加加速时间--------------------------------------------------- = " + JSON.stringify(this.game_speed_list));
        }
        ...
        public setTempSpeed(value) 
        {
            if (this.game_temp_speed != value){
                ...
                QEventMgr.instance.sendEvent(QEventType.MAIN_VIEW_RESTART_TIMER);
            }
        }
        public coseMoney(mtype:number,count:number):boolean
        {
            if(mtype==QGameConst.QMoneyType.Coin)
            {
                return this.coseCoin(count);
            }
            else if(mtype==QGameConst.QMoneyType.Diamond)
            {
                return this.coseDiamond(count);
            }
            else if(mtype==QGameConst.QMoneyType.Video)
            {
                return true;
            }
        }
        public coseCoin(count: number): boolean
        {
            ...
            if (!QUtil.StrJudge(this.game_gold + "", count + ""))
            {
                QDebug.log("Not enough coin !")
                return false;
            }
            else
            {
        ...
        {
            var car_data = QCfgMgr.instance.getCarRandom();
        ...
        public initAirDropList(){
            // 处理之前的空投数据
            QSavedDateItem.instance.setItemToLocalStorage("AIR_DROP_STATE",  "HIDE" );
            this.AirDropList = JSON.parse(QSavedDateItem.instance.getItemFromLocalStorage("QMergeDateAirDropList",JSON.stringify([]))); 
            this.AirDropIsOpen = parseInt(QSavedDateItem.instance.getItemFromLocalStorage("QMergeDateAirDropIsOpen","2")); 
            this.FreeAirDropOpen = parseInt(QSavedDateItem.instance.getItemFromLocalStorage("QMergeDateFreeAirDropOpen","1")); 
            QEventMgr.instance.register(QEventType.ON_CREATE_AIRDROP, this.onCreateAirDrop, this);
            this.AirDropCount = parseInt(QSavedDateItem.instance.getItemFromLocalStorage("QMergeDateAirDropCount","0"));
            this.AirdropPos = JSON.parse(QSavedDateItem.instance.getItemFromLocalStorage("QMergeDateAirDropPos",JSON.stringify({x:0, y:0}))); 
            this.AirDropLevel = 15;
            if (this.AirDropCount == 0){
                QSavedDateItem.instance.setItemToLocalStorage("QMergeDateAirDropIsOpen","2"); 
                this.AirDropIsOpen = 2;
            }
        }
        ...
        public createAirDrop() {
            ...
            if(QMergeData.instance.CarMaxLevel <= QMergeData.instance.AirDropLevel) {
                ...
                if (index == -1) {
                    ...
                    QEventMgr.instance.sendEvent(QEventType.MAIN_VIEW_TOUCH_UP_CAR_INDEX, { "index": index});  //更新车的位置
                    QDebug.D("空投箱---------------------------");
                }
            }else {
                QDebug.D("空投箱-------普通--------------------");
                // 普通
                QEventMgr.instance.sendEvent(QEventType.MAIN_OPEN_AIRDROP);
            }
        ...
        public onCalculateAirDrop() {
            ...
            QSavedDateItem.instance.setItemToLocalStorage("QMergeDateAirDropList",JSON.stringify(QMergeData.instance.AirDropList)); 
        }
        /**
         * 更具数据表生成空投数据
         */
        public onCreateAirdropData(AirDropCount){
            ...
            var dataByCar = QCfgMgr.instance.getCarInfoById(this.CarMaxLevel);
       ...
       public judgeCarDifferent():any {
        ...
        public initAirDropTime(){
            ...
            var time = parseInt(QSavedDateItem.instance.getItemFromLocalStorage("QMergeDateAirDropTime", this.AirDropTime + ""));
            if (time == 0){
                this.createAirDrop();
                return;
            }
            ...
            var loopAirdropTime = function(){
                ...
                // QDebug.D("空投定时器 ----------" + time);
                if (time > 0){
                    QSavedDateItem.instance.setItemToLocalStorage("QMergeDateAirDropTime",time + "");
                }else 
                {
                    Laya.timer.clear(that,loopAirdropTime);
                    that.createAirDrop();
                    QSavedDateItem.instance.setItemToLocalStorage("QMergeDateAirDropTime", that.AirDropTime + "");
                }
            }
            loopAirdropTime();
        ...
        public getAirDropLevel():any{
            if (QMergeData.instance.AirDropList.length > 0){
                var list = this.judgeCarDifferent()
                var level = QMergeData.instance.AirDropList[0];
                if (list != -1) {
                    level = list[0].level;
                }
                QMergeData.instance.AirDropList.splice(0,1);
                this.AirDropCount = QMergeData.instance.AirDropList.length;
                QSavedDateItem.instance.setItemToLocalStorage("QMergeDateAirDropCount", this.AirDropCount + "");
                QSavedDateItem.instance.setItemToLocalStorage("QMergeDateAirDropList",JSON.stringify(QMergeData.instance.AirDropList)); 
                return level;
            }
        ...
        public setFreeAirDropOpen(){
            QSavedDateItem.instance.setItemToLocalStorage("QMergeDateFreeAirDropOpen","0"); 
        ...
        public onCreateAirDrop(){
            ...
            if (QMergeData.instance.AirDropIsOpen != 1) {
                ...
                if(QMergeData.instance.AirDropIsOpen == 3){
                    QDebug.D("停车位已满 空投未开启-----------------");
                    // 未开启
                    this.createAirDrop();
                }
                return;
            ...
            var _index = QMergeData.instance.JudgeSolt();
            if (_index != -1){
                var level = QMergeData.instance.getAirDropLevel();
                if (level != -1){
                    var data_1: any = {};
                    data_1.level = level;
                    data_1.state = 5; // 1 直接显示  0 显示箱子  2 缩小动作 3 空投箱 4空投箱降落 5 空投移动车辆
                    QMergeData.instance.changeCarSlotData(_index, data_1);
                    QEventMgr.instance.sendEvent(QEventType.MAIN_VIEW_TOUCH_UP_CAR_INDEX, { "index": _index, pos: this.AirdropPos});  //更新车的位置
                    ...
                    if (QMergeData.instance.AirDropList.length > 0){
                        ...
                        // 删除数据
                        // 普通
                        QEventMgr.instance.sendEvent(QEventType.CLOSE_AIRDROP);
                        var car_slot_index = this.judgeCarSlotDataAirdrop();
                        QMergeData.instance.changeCarSlotData(car_slot_index, null);
                        QEventMgr.instance.sendEvent(QEventType.MAIN_VIEW_TOUCH_UP_CAR_INDEX, { "index": car_slot_index});
...
export default game.data.QMergeData;